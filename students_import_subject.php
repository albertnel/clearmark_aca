<?php
	...
	if (sizeof($_FILES) > 0)
	{
		...
		while($contents = fgetcsv($fh))
		{
			if (itemImportEntryIsValid($contents))
			{
				...
				$already = $DB->getcell("SELECT COUNT(*) FROM user_rel_subject WHERE user_id = ".$user_id." AND subject_id = ".$subject_id);
				if (!$already) $DB->put("INSERT INTO user_rel_subject VALUES(".$user_id.", ".$subject_id.", '1')");
			}
			else
			{
				$invalid++;
	...
	$html = file_get_contents('students_import_subject.html');
