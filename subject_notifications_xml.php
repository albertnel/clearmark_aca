<?php
	...
	$rows = $DB->get("SELECT * FROM notification
							...
							WHERE subject_id = ".$_SESSION['subject']['subject_id']);
	if($qtype && $query)
	{
	...
	foreach($rows as $row)
	{
		...
		$xml .= "<cell><![CDATA[<a onclick='window.location=\"subject_notification_manage.php?".$row['notification_id']."\";'>Edit</a> || <a onclick='deleteNotification(".$row['notification_id'].");'>Delete</a>]]></cell>";
		$xml .= "</row>";
	}
	$xml .= "</rows>";
	echo $xml;
