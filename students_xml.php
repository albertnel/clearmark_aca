<?php
	...
	$rows = $DB->get("SELECT user_profile.user_id, username, name, surname, email FROM user_profile
	...
	foreach($rows as $row)
	{
		...
		$xml .= "<cell><![CDATA[<a onclick='window.location=\"message_view.php?+".$row['user_id']."\";'>Send message</a> || <a onclick='window.location=\"student_manage.php?".$row['user_id']."\";'>Manage</a> || <a onclick='deleteUser(".$row['user_id'].");'>Delete</a>]]></cell>";
		$xml .= "</row>";
	}
	$xml .= "</rows>";
	echo $xml;
