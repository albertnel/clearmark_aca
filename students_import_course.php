<?php
	...
	function itemImportEntryIsValid($contents)
	{
		...
		if (($contents[0] != "") && ($contents[1] != ""))
		{
			...
			$count = $DB->getcell("SELECT COUNT(*) FROM course WHERE course_name = '".$contents[1]."'");
			if ($count == 0)
				return 0;
			else
				return 1;
	...
	if (sizeof($_FILES) > 0)
	{
		...
		while($contents = fgetcsv($fh))
		{
			if (itemImportEntryIsValid($contents))
			{
				...
				$course_id = $DB->getcell("SELECT course_id FROM course WHERE course_name = '".$contents[1]."'");

				$already = $DB->getcell("SELECT COUNT(*) FROM user_rel_course WHERE user_id = ".$user_id." AND course_id = ".$course_id);
				if (!$already) $DB->put("INSERT INTO user_rel_course VALUES(".$user_id.", ".$course_id.", '1')");
			}
			else
			{
				$invalid++;
		...
		// -- Results --
		$parse['import_result_msg'] = '	<div class="text_normal" style="padding-top:10px;">'.$valid.' students have been assigned successfully.</div>';
	...
	$html = file_get_contents('students_import_course.html');
